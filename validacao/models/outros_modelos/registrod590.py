...
class RegistroComunicacaoD590(models.Model):

    empresa = models.ForeignKey(Empresa, on_delete=models.CASCADE, verbose_name='Empresa', null=True, blank=True)

    registro_d500 = models.ForeignKey(
        RegistroComunicacaoD500,
        on_delete=models.CASCADE,
        related_name="analiticos",
        verbose_name="Documento de Comunicação (D500)"
    )
    ...
    # CFOP: Código Fiscal de Operações e Prestações (4 caracteres, ex: 1302)
    ...
    def __str__(self):
        return f"D590 - CFOP {self.cfop} - CST {self.cst_icms} (Doc: {self.registro_d500.num_doc})"
